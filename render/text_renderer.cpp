...
namespace browser::render {
    ...
    f32 TextRenderer::render_text(
        Renderer *r, const std::string &text, f32 x, f32 y, const Color &color, u32 pixel_size) {
        f32 cursor_x = x;
        ...
        while (offset < len) {
            ...
            if (face_)
                prev_gid = face_->glyph_index(dr.codepoint);
            ...
            r->add_tex_quad(glyph_x, glyph_y, (f32)gt->width, (f32)gt->height, color, gt->u0, gt->v0, gt->u1, gt->v1);
    ...
    TextRenderer::GlyphAtlasEntry *TextRenderer::prepare_glyph(u32 codepoint, u32 pixel_size) {
        ...
        auto rasterize = [&]() -> Result<GlyphBitmap> {
            if (!face_)
                return Result<GlyphBitmap>(std::string("no font face"));
            if (fm_) {
