...
void Canvas2D::stroke_rect(f32 x, f32 y, f32 w, f32 h) {
    ...
    f32 x1 = x + w, y1 = y + h;
    begin_path();
    move_to(x, y); line_to(x1, y); line_to(x1, y1); line_to(x, y1); close_path();
    stroke();
}
...
void Canvas2D::fill_text(const std::string& text, f32 x, f32 y) {
    ...
    if (sz_pos != std::string::npos && sz_pos > 0) {
        ...
        if (end != num_str.c_str()) size = parsed;
    }
    ...
    for (char ch : text) {
        ...
        if (face) {
            ...
            if (metrics.is_ok()) cur_x += metrics.unwrap().advance_x;
            else cur_x += size * 0.6f;
        } else {
            cur_x += size * 0.6f;
        }
    }
...
void Canvas2D::add_bounded_arc(f32 cx, f32 cy, f32 r, f32 a0, f32 a1, bool ccw) {
    if (r <= 0) return;
    f32 da = 0.05f;
    if (ccw) { if (a1 > a0) a1 -= 2 * 3.14159f; }
    else { if (a1 < a0) a1 += 2 * 3.14159f; }
...
void Canvas2D::bezier_split(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2, f32 x3, f32 y3, int depth) {
    ...
    f32 flat = std::abs(x1 - x0) + std::abs(y1 - y0) +
...
void Canvas2D::close_path() {
    if (!current_path_.empty() && !sub_paths_.empty()) {
        auto& first_sub = sub_paths_.back();
        if (!first_sub.empty()) {
            current_path_.push_back(first_sub[0]);
            cx_ = first_sub[0].x; cy_ = first_sub[0].y;
        }
    }
}
...
void Canvas2D::draw_line_low(f32 x0, f32 y0, f32 x1, f32 y1) {
    ...
    f32 y = y0;
    for (f32 x = x0; x <= x1; x++) {
        set_pixel(static_cast<u32>(x), static_cast<u32>(y),
            static_cast<u8>(stroke_r_ * 255), static_cast<u8>(stroke_g_ * 255),
            static_cast<u8>(stroke_b_ * 255), static_cast<u8>(stroke_a_ * global_alpha_ * 255));
        if (D > 0) { y += yi; D -= 2 * dx; }
        D += 2 * dy;
    }
}
...
void Canvas2D::fill() {
    ...
    for (auto& pt : current_path_)
        full_path.push_back(pt);
    if (full_path.size() < 3) return;
...
void Canvas2D::save() {
    State s;
    s.fill_r = fill_r_; s.fill_g = fill_g_; s.fill_b = fill_b_; s.fill_a = fill_a_;
    s.stroke_r = stroke_r_; s.stroke_g = stroke_g_; s.stroke_b = stroke_b_; s.stroke_a = stroke_a_;
    s.global_alpha = global_alpha_; s.line_width = line_width_;
    s.font = font_; s.text_align = text_align_; s.text_baseline = text_baseline_;
    s.transform = transform_;
    saved_states_.push_back(s);
}

void Canvas2D::restore() {
    if (saved_states_.empty()) return;
    auto& s = saved_states_.back();
    fill_r_ = s.fill_r; fill_g_ = s.fill_g; fill_b_ = s.fill_b; fill_a_ = s.fill_a;
    stroke_r_ = s.stroke_r; stroke_g_ = s.stroke_g; stroke_b_ = s.stroke_b; stroke_a_ = s.stroke_a;
    global_alpha_ = s.global_alpha; line_width_ = s.line_width;
    font_ = s.font; text_align_ = s.text_align; text_baseline_ = s.text_baseline;
...
void Canvas2D::rotate(f32 angle) {
    ...
    transform_.a = a; transform_.b = b; transform_.c = c2; transform_.d = d;
}

void Canvas2D::scale(f32 x, f32 y) {
    transform_.a *= x; transform_.b *= x;
    transform_.c *= y; transform_.d *= y;
}
...
void Canvas2D::transform(f32 a, f32 b, f32 c, f32 d, f32 e, f32 f) {
    ...
    transform_.a = na; transform_.b = nb;
    transform_.c = nc; transform_.d = nd;
    transform_.e = ne; transform_.f = nf;
}

void Canvas2D::set_transform(f32 a, f32 b, f32 c, f32 d, f32 e, f32 f) {
    transform_.a = a; transform_.b = b;
    transform_.c = c; transform_.d = d;
    transform_.e = e; transform_.f = f;
}

void Canvas2D::set_fill_style(f32 r, f32 g, f32 b, f32 a) {
    fill_r_ = r; fill_g_ = g; fill_b_ = b; fill_a_ = a;
}

void Canvas2D::set_stroke_style(f32 r, f32 g, f32 b, f32 a) {
    stroke_r_ = r; stroke_g_ = g; stroke_b_ = b; stroke_a_ = a;
}
...
FontFace* Canvas2D::resolve_font() {
    if (!font_manager_) return nullptr;
...
void Canvas2D::rasterize_glyph(u32 codepoint, f32 x, f32 y, f32 size, bool fill) {
    ...
    for (u32 row = 0; row < gb.height; row++) {
        for (u32 col = 0; col < gb.width; col++) {
            ...
            if (dx >= width_ || dy >= height_) continue;
...
void Canvas2D::put_image_data(const std::vector<u8>& data, u32 dx, u32 dy, u32 dw, u32 dh) {
    ...
    for (u32 row = 0; row < dh; row++) {
        for (u32 col = 0; col < dw; col++) {
            u32 dst_x = dx + col;
            u32 dst_y = dy + row;
            if (dst_x >= width_ || dst_y >= height_) continue;
...
std::string Canvas2D::to_data_url() const {
    ...
    for (u32 y = 0; y < height_; y++) {
        ...
        for (u32 x = 0; x < width_ * 4; x++)
            png.push_back(pixels_[row_start + x]);
    }
