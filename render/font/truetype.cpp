...
namespace browser::render {
    ...
    Result<void> FontFace::load_tables_from_offset(const u8 *data, u32 total_size, u32 font_offset) {
        ...
        if (fl < 12)
            return Result<void>(std::string("Font too small"));

        u32 num_tables = read_u16_be(fp, 4, fl);
        if (num_tables > 100)
            return Result<void>(std::string("Too many tables"));
        ...
        for (u32 i = 0; i < num_tables; i++) {
            ...
            if (abs_off > total_size || tab_len > total_size || abs_off + tab_len > total_size)
                continue;

            if (std::strcmp(tag, "cmap") == 0) {
                cmap_off_ = abs_off;
                cmap_len_ = tab_len;
            } else if (std::strcmp(tag, "head") == 0) {
                head_off_ = abs_off;
                head_len_ = tab_len;
            } else if (std::strcmp(tag, "hhea") == 0) {
                hhea_off_ = abs_off;
                hhea_len_ = tab_len;
            } else if (std::strcmp(tag, "maxp") == 0) {
                maxp_off_ = abs_off;
                maxp_len_ = tab_len;
            } else if (std::strcmp(tag, "hmtx") == 0) {
                hmtx_off_ = abs_off;
                hmtx_len_ = tab_len;
            } else if (std::strcmp(tag, "loca") == 0) {
                loca_off_ = abs_off;
                loca_len_ = tab_len;
            } else if (std::strcmp(tag, "glyf") == 0) {
                glyf_off_ = abs_off;
                glyf_len_ = tab_len;
            } else if (std::strcmp(tag, "kern") == 0) {
                kern_off_ = abs_off;
                kern_len_ = tab_len;
            }
        }
        ...
        if (num_glyphs_ == 0)
            return Result<void>(std::string("No glyphs"));

        num_h_metrics_ = read_u16_be(fp, hhea_off_ - font_offset + 34, fl);
        if (num_h_metrics_ == 0)
            num_h_metrics_ = num_glyphs_;
    ...
    Result<GlyphOutline> FontFace::parse_simple_glyph(const u8 *gdata, u32 gsize, i16 num_contours, int bezier_steps) {
    ...
    Result<GlyphOutline> FontFace::read_outline(u16 glyph_idx, int bezier_steps) const {
        ...
        while (comp_off + 4 <= gsize) {
            ...
            if (comp.num_contours <= 0 && comp.points.empty())
                continue;
            ...
            if (flags & 0x0002) {
                ...
                if (parent_pt_idx * 2 + 1 < result.points.size() && child_pt_idx * 2 + 1 < comp.points.size()) {
    ...
    Result<FontFace::GlyphMetrics> FontFace::get_metrics_by_gid(u16 gid, u32 pixel_size) const {
        ...
        u32 fl = (u32)font_data_.size();
        if (units_per_em_ <= 0)
            return Result<GlyphMetrics>(std::string("invalid unitsPerEm"));
