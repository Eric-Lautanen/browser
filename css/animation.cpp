...
void AnimationEngine::add_animation(const std::string& element_key, AnimationState anim) {
    auto it = keyframes_map_.find(anim.name);
    if (it != keyframes_map_.end()) {
        anim.keyframes = &it->second;
    }
    if (anim.duration <= 0) anim.duration = 0.001f;
...
f32 AnimationEngine::cubic_bezier(f32 t, f32 x1, f32 y1, f32 x2, f32 y2) const {
    ...
    f32 x = t;
    for (int i = 0; i < 8; i++) {
        f32 cx = 3.0f * (1.0f - x) * (1.0f - x) * x * x1 +
                 3.0f * (1.0f - x) * x * x * x2 +
                 x * x * x;
        f32 dx = 3.0f * (1.0f - x) * (1.0f - x) * x1 +
                 6.0f * (1.0f - x) * x * (x2 - x1) +
                 3.0f * x * x * (1.0f - x2);
        if (std::abs(dx) < 1e-6f) break;
        x -= (cx - t) / dx;
    }
    return 3.0f * (1.0f - x) * (1.0f - x) * x * y1 +
           3.0f * (1.0f - x) * x * x * y2 +
           x * x * x;
}
...
f32 AnimationEngine::apply_timing_function(f32 t, const std::string& func) const {
    ...
    if (func == "ease") return cubic_bezier(t, 0.25f, 0.1f, 0.25f, 1.0f);
    if (func == "ease-in") return cubic_bezier(t, 0.42f, 0.0f, 1.0f, 1.0f);
    if (func == "ease-out") return cubic_bezier(t, 0.0f, 0.0f, 0.58f, 1.0f);
    if (func == "ease-in-out") return cubic_bezier(t, 0.42f, 0.0f, 0.58f, 1.0f);
    // steps() - simplified
...
f32 AnimationEngine::apply_direction(f32 t, const std::string& direction, f32 iteration) const {
    ...
    else if (direction == "alternate") reverse = (iter_int % 2 == 1);
    else if (direction == "alternate-reverse") reverse = (iter_int % 2 == 0);
    ...
    if (from.type == to.type) {
        switch (from.type) {
            ...
            case CSSValue::Type::COLOR:
                result.color.r = static_cast<u8>(std::max(0.0f, std::min(255.0f,
                    from.color.r + (to.color.r - from.color.r) * t)));
                result.color.g = static_cast<u8>(std::max(0.0f, std::min(255.0f,
                    from.color.g + (to.color.g - from.color.g) * t)));
                result.color.b = static_cast<u8>(std::max(0.0f, std::min(255.0f,
                    from.color.b + (to.color.b - from.color.b) * t)));
                result.color.a = static_cast<u8>(std::max(0.0f, std::min(255.0f,
                    from.color.a + (to.color.a - from.color.a) * t)));
                break;
...
void AnimationEngine::update(f32 dt) {
    ...
    for (auto& [key, anim] : animations_) {
        ...
        if (iteration_duration <= 0) iteration_duration = 0.001f;

        f32 total_duration = iteration_duration * anim.iteration_count;
        if (anim.elapsed >= total_duration && anim.iteration_count > 0) {
            anim.finished = true;
...
std::vector<std::pair<std::string, std::vector<Declaration>>>
AnimationEngine::get_interpolated_declarations() const {
    std::vector<std::pair<std::string, std::vector<Declaration>>> result;

    for (const auto& [key, anim] : animations_) {
        if (!anim.keyframes || anim.keyframes->blocks.empty()) continue;
        ...
        for (size_t bi = 0; bi + 1 < anim.keyframes->blocks.size(); bi++) {
            const auto& block_a = anim.keyframes->blocks[bi];
            const auto& block_b = anim.keyframes->blocks[bi + 1];

            if (block_a.positions.empty() || block_b.positions.empty()) continue;
            ...
            for (const auto& da : block_a.declarations) {
                for (const auto& db : block_b.declarations) {
                    if (da.property != db.property) continue;
                    Declaration interpolated;
                    interpolated.property = da.property;
                    if (!da.values.empty() && !db.values.empty()) {
                        interpolated.values.push_back(
                            interpolate_value(da.values[0], db.values[0], t));
    ...
    for (const auto& [key, anim] : finished_animations_) {
        if (anim.fill_mode == "forwards" || anim.fill_mode == "both") {
            if (!anim.keyframes || anim.keyframes->blocks.empty()) continue;
            const auto& last_block = anim.keyframes->blocks.back();
            std::vector<Declaration> decls;
...
f32 TransitionManager::apply_timing_function(f32 t, const std::string& func) const {
    if (func == "linear") return t;
    if (func == "ease") { f32 x = t; return x * x * (3.0f - 2.0f * x); } // smoothstep
    if (func == "ease-in") return t * t;
    if (func == "ease-out") return t * (2.0f - t);
    if (func == "ease-in-out") return t < 0.5f ? 2.0f * t * t : -1.0f + (4.0f - 2.0f * t) * t;
    return t;
}

void TransitionManager::on_style_change(
    ...
    const std::string& timing_func)
{
    ...
    for (auto& ts : element_transitions) {
        ...
        if (it != new_style.end()) {
            ...
            bool changed = false;
            if (it->second.type != ts.to_value.type) changed = true;
            else if (it->second.type == CSSValue::Type::NUMBER && it->second.number != ts.to_value.number) changed = true;
            else if (it->second.type == CSSValue::Type::LENGTH && it->second.length.value != ts.to_value.length.value) changed = true;
            else if (it->second.type == CSSValue::Type::COLOR &&
                     (it->second.color.r != ts.to_value.color.r ||
                      it->second.color.g != ts.to_value.color.g ||
                      it->second.color.b != ts.to_value.color.b)) changed = true;
    ...
    for (const auto& [prop, val] : new_style) {
        if (std::find(handled.begin(), handled.end(), prop) != handled.end()) continue;
        auto old_it = old_style.find(prop);
        if (old_it != old_style.end()) {
            // Value changed, start transition
            bool changed = false;
            if (old_it->second.type != val.type) changed = true;
            else if (val.type == CSSValue::Type::NUMBER && old_it->second.number != val.number) changed = true;
            else if (val.type == CSSValue::Type::LENGTH && old_it->second.length.value != val.length.value) changed = true;
            else if (val.type == CSSValue::Type::COLOR &&
                     (old_it->second.color.r != val.color.r ||
                      old_it->second.color.g != val.color.g ||
                      old_it->second.color.b != val.color.b)) changed = true;
