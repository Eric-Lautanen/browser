...
bool DownloadManager::should_download(const std::string& url, const std::string& content_disposition,
...
async::task<Result<void>> DownloadManager::start_download(const std::string& url) {
    ...
    if (filename.empty()) {
        auto slash_pos = url.rfind('/');
        if (slash_pos != std::string::npos) filename = url.substr(slash_pos + 1);
        auto qpos = filename.find('?');
    ...
    HANDLE hZone = CreateFileA(zone_path.c_str(), GENERIC_WRITE, 0, nullptr,
...
bool DownloadManager::is_mime_mismatch(const std::string& filename, const std::string& mime_type) {
    ...
    if (ext == ".exe" && mime_type.find("image/") == 0) return true;
    if (ext == ".dll" && mime_type.find("image/") == 0) return true;
    if (ext == ".scr" && mime_type.find("image/") == 0) return true;
