...
namespace browser {
    ...
    ThemeMode SettingsManager::theme() const {
        return theme_;
    }
    void SettingsManager::set_theme(ThemeMode mode) {
        theme_ = mode;
    }
    ...
    void SettingsManager::set_search_engine(const std::string &engine) {
        search_engine_ = engine;
    }

    std::string SettingsManager::cookie_policy() const {
        return cookie_policy_;
    }
    void SettingsManager::set_cookie_policy(const std::string &policy) {
        cookie_policy_ = policy;
    }

    u32 SettingsManager::cache_size_mb() const {
        return cache_size_mb_;
    }
    void SettingsManager::set_cache_size_mb(u32 mb) {
        cache_size_mb_ = mb;
    }

    u32 SettingsManager::font_size() const {
        return font_size_;
    }
    void SettingsManager::set_font_size(u32 size) {
        font_size_ = size;
    }

    f32 SettingsManager::zoom_level() const {
        return zoom_level_;
    }
    void SettingsManager::set_zoom_level(f32 level) {
        zoom_level_ = level;
    }

    std::string SettingsManager::download_behavior() const {
        return download_behavior_;
    }
    void SettingsManager::set_download_behavior(const std::string &b) {
        download_behavior_ = b;
    }
    ...
    Result<void> SettingsManager::load_from_file(const std::string &path) {
        ...
        if (!f)
            return std::string("failed to read version");

        if (version == 1) {
            u32 field_count = 0;
            f.read(reinterpret_cast<char *>(&field_count), sizeof(field_count));
            if (!f)
                return std::string("failed to read field count");

            for (u32 i = 0; i < field_count; i++) {
                std::string name, value;
                if (!read_setting_field(f, name, value))
                    break;
                ...
                if (name == "theme") {
                    ...
                    if (end && end != value.c_str())
                        cache_size_mb_ = static_cast<u32>(parsed);
        ...
        if (p1 == std::string::npos)
            return std::string("bad format: no first delimiter");
        auto p2 = line.find('|', p1 + 1);
        if (p2 == std::string::npos)
            return std::string("bad format: no second delimiter");
