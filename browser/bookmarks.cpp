...
Result<void> BookmarkManager::save_to_file(const std::string& path) {
    ...
    if (!file.is_open()) return std::string("Failed to open file for writing: " + path);
    for (const auto& b : bookmarks_) {
        file << escape_pipe(b.title) << '|' << escape_pipe(b.url) << '|'
             << b.added_at << '|' << escape_pipe(b.folder) << '\n';
    }
    file.close();
    if (!file.good()) return std::string("Failed to write file: " + path);
    return {};
}

Result<void> BookmarkManager::load_from_file(const std::string& path) {
    std::ifstream file(path);
    if (!file.is_open()) return std::string("Failed to open file for reading: " + path);
    bookmarks_.clear();
    std::string line;
    while (std::getline(file, line)) {
        ...
        if (parts.size() > 4) {
            ...
            if (endp == parts[2].c_str()) b.added_at = 0;
            if (parts.size() >= 4) b.folder = parts[3];
        }
