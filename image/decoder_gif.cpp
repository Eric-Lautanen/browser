...
#pragma pack(pop)
        ...
        Result<Image> decode(const u8 *data, size_t size) override {
            ...
            while (pos < size) {
                ...
                if (block_type == 0x21) {
                    ...
                    // Image Descriptor
                    if (pos + sizeof(GIFImageDescriptor) > size)
                        break;
                    ...
                    // LZW Decompress
                    std::vector<u8> indexed = lzw_decompress(
                        lzw_data, min_code_size, static_cast<u32>(id.width) * static_cast<u32>(id.height));
    ...
    private:
        std::vector<u8> lzw_decompress(const std::vector<u8> &data, u8 min_code_size, u32 max_pixels) {
            ...
            // I-perf: prefix/suffix-chained table. The previous representation
            // stored every table entry as its own heap vector and copied the
            // full string on each decode step — quadratic allocator churn.
            // Flat parallel arrays grown lazily (reserved once) replace both.
            constexpr u32 kMaxLzwCodes = 4096;
            constexpr u16 kNoPrefix = 0xFFFF;
            std::vector<u16> prefix;
            std::vector<u8> suffix;
            prefix.reserve(kMaxLzwCodes);
            suffix.reserve(kMaxLzwCodes);
            for (u32 i = 0; i < clear_code; i++) {
                prefix.push_back(kNoPrefix);
                suffix.push_back(static_cast<u8>(i));
            }
            ...
            while (output.size() < max_pixels) {
                ...
                if (first) {
                    if (code >= clear_code)
                        break;  // first code must be a root
                    output.push_back(suffix[code]);
                    old_code = code;
                    first = false;
                    continue;
                }

                // Decode the string for this code. KwKwK (code == next_code)
                // decodes as the previous string plus its own first byte.
                bool kwkw = (code == next_code);
                if (code > next_code)
                    break;  // corrupt stream

                u32 src = kwkw ? old_code : code;
                u8 scratch[kMaxLzwCodes];
                u32 n = 0;
                u32 c = src;
                while (c != kNoPrefix) {
                    if (n >= kMaxLzwCodes)
                        return {};  // cannot happen with well-formed tables
                    scratch[n++] = suffix[c];
                    c = prefix[c];
                }
                for (u32 i = n; i-- > 0;) {
                    if (output.size() >= max_pixels)
                        break;
                    output.push_back(scratch[i]);
                }
                u8 entry_first = scratch[n - 1];
                if (kwkw && output.size() < max_pixels)
                    output.push_back(entry_first);

                // New table entry: previous string plus first byte of this one.
                if (next_code < kMaxLzwCodes) {
                    prefix[next_code] = static_cast<u16>(old_code);
                    suffix[next_code] = entry_first;
                    next_code++;
