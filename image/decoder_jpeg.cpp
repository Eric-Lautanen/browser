...
namespace browser::image {
    ...
    static const u8 ZIGZAG[64] = {0,  1,  8,  16, 9,  2,  3,  10, 17, 24, 32, 25, 18, 11, 4,  5,
    ...
    // I-perf: precomputed cosine basis. The naive path evaluated std::cos for
    // every (x,u) product of every 1-D pass — ~900 transcendental calls per
    // 8×8 block. The basis is constant; build it once.
    struct JpegCosBasis {
        // c[x][u] = C(u) * cos((2x+1)*u*pi/16), with C(0)=1/√2, C(u>0)=1
        float c[8][8];
        JpegCosBasis() {
            const float PI_16 = 3.14159265358979323846f / 16.0f;
            for (int x = 0; x < 8; x++) {
                for (int u = 0; u < 8; u++) {
                    float cu = (u == 0) ? 0.70710678118654752440f : 1.0f;
                    c[x][u] = cu * std::cos((2.0f * x + 1.0f) * u * PI_16);
                }
            }
        }
    };
    static const JpegCosBasis kJpegCos;
    ...
    static void idct_1d(const float *in, float *out) {
        for (int x = 0; x < 8; x++) {
            const float *basis = kJpegCos.c[x];
            float sum = 0.0f;
            for (int u = 0; u < 8; u++) sum += in[u] * basis[u];
            out[x] = sum;
        }
    }
    ...
    static void idct_2d(const float *in, float *out) {
        float rows[8][8];
        for (int y = 0; y < 8; y++) idct_1d(in + y * 8, rows[y]);

        float col[8];
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) col[y] = rows[y][x];
            idct_1d(col, col);                                            // in-place
            for (int y = 0; y < 8; y++) out[y * 8 + x] = col[y] * 0.25f;  // final (1/4) factor
        }
    }
    ...
    private:
        ...
        bool read_dqt() {
            ...
            while (pos_ < end) {
                ...
                if (prec == 0) {
                    if (pos_ + 64 > size_)
                        return false;
                    for (int i = 0; i < 64; i++) q_tables_[tid][ZIGZAG[i]] = data_[pos_++];
                } else {
                    if (pos_ + 128 > size_)
                        return false;
                    for (int i = 0; i < 64; i++) {
                        q_tables_[tid][ZIGZAG[i]] =
                            static_cast<u16>((static_cast<u16>(data_[pos_]) << 8) | data_[pos_ + 1]);
                        pos_ += 2;
                    }
                }
            }
        ...
        bool read_sof() {
            ...
            if (comp_count_ < 1 || comp_count_ > 4)
                return false;
            ...
            for (int i = 0; i < comp_count_; i++) {
                ...
                if (comps_[i].q_table_idx >= 4)
                    return false;
                pos_++;
                if (comps_[i].h_samp > h_samp_max_)
                    h_samp_max_ = comps_[i].h_samp;
                if (comps_[i].v_samp > v_samp_max_)
                    v_samp_max_ = comps_[i].v_samp;
            }
        ...
        bool read_dht() {
            ...
            while (pos_ + 17 <= end && pos_ + 17 <= size_) {
                ...
                for (u32 i = 0; i < total; i++) tbl.values[i] = data_[pos_++];
                tbl.build();
            }
            return true;
        }
                    ...
                    for (int c = 0; c < comp_count_; c++) {
                        ...
                        for (int vb = 0; vb < ci.v_samp; vb++) {
                            for (int hb = 0; hb < ci.h_samp; hb++) {
                                ...
                                for (int i = 0; i < 64; i++) block[i] *= static_cast<float>(qt[i]);
