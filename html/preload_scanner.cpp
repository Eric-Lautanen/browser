...
void PreloadScanner::scan_token(const Token& token, const std::string& base_url) {
    if (token.index() != 1) return; // Only process TagToken
    const auto& tag = std::get<TagToken>(token);
    if (tag.type != TokenType::START_TAG) return;

    auto get_attr = [&](const std::string& name) -> std::string {
        for (const auto& attr : tag.attributes) {
            std::string attr_name_lower;
            for (char c : attr.name) attr_name_lower += static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
            if (attr_name_lower == name) return attr.value;
        }
        return {};
    };
