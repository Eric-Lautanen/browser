...
namespace browser::html {
    ...
    bool Parser::has_element_in_scope(const std::string &tag_name, const std::vector<std::string> &extras) {
        static const std::unordered_set<std::string> scope_tags = {"applet",
                                                                   "caption",
                                                                   "html",
                                                                   "table",
                                                                   "td",
        ...
        for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
            if (!stack_[i])
                continue;
            if (stack_[i]->tag_name == tag_name)
                return true;
            if (scope_tags.find(stack_[i]->tag_name) != scope_tags.end())
                return false;
            for (const auto &e : extras) {
                if (stack_[i]->tag_name == e)
                    return false;
            }
        }
        return false;
    ...
    bool Parser::has_element_in_list_scope(const std::string &tag_name) {
        static const std::unordered_set<std::string> list_scope = {
            ...
            "annotation-xml", "foreignobject", "desc", "title", "ol", "ul"};
        for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
            if (!stack_[i])
                continue;
            if (stack_[i]->tag_name == tag_name)
                return true;
            if (list_scope.find(stack_[i]->tag_name) != list_scope.end())
                return false;
        }
        return false;
    }
    ...
    bool Parser::has_element_in_button_scope(const std::string &tag_name) {
        static const std::unordered_set<std::string> button_scope = {"applet",
                                                                     "caption",
                                                                     "html",
                                                                     "table",
                                                                     "td",
        ...
        for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
            ...
            if (stack_[i]->tag_name == tag_name)
                return true;
            if (button_scope.find(stack_[i]->tag_name) != button_scope.end())
                return false;
        }
        return false;
    }
    ...
    bool Parser::has_element_in_table_scope(const std::string &tag_name) {
        static const std::unordered_set<std::string> table_scope = {"applet",
                                                                    "caption",
                                                                    "html",
                                                                    "table",
                                                                    "td",
        ...
        for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
            ...
            if (stack_[i]->tag_name == tag_name)
                return true;
            if (table_scope.find(stack_[i]->tag_name) != table_scope.end())
                return false;
        }
        return false;
    }
    ...
    bool Parser::has_element_in_select_scope(const std::string &tag_name) {
        static const std::unordered_set<std::string> select_scope_pass = {"optgroup", "option"};
        for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
            if (!stack_[i])
                continue;
            if (stack_[i]->tag_name == tag_name)
                return true;
            if (select_scope_pass.find(stack_[i]->tag_name) == select_scope_pass.end())
                return false;
        }
        return false;
    }
    ...
    void Parser::clear_stack_back_to_table_context() {
        flush_pending_text();
        while (current_node() && current_node()->tag_name != "table" && current_node()->tag_name != "template" &&
               current_node()->tag_name != "html") {
            stack_.pop_back();
        }
    }
    ...
    void Parser::clear_stack_back_to_table_body_context() {
        flush_pending_text();
        while (current_node() && current_node()->tag_name != "tbody" && current_node()->tag_name != "tfoot" &&
               current_node()->tag_name != "thead" && current_node()->tag_name != "template" &&
               current_node()->tag_name != "html") {
            stack_.pop_back();
        }
    }
    ...
    void Parser::clear_stack_back_to_table_row_context() {
        flush_pending_text();
        while (current_node() && current_node()->tag_name != "tr" && current_node()->tag_name != "template" &&
               current_node()->tag_name != "html") {
            stack_.pop_back();
        }
    }
    ...
    bool is_void_element(const std::string &tag) {
        static const std::unordered_set<std::string> voids = {"area",
                                                              "base",
                                                              "br",
                                                              "col",
                                                              "embed",
    ...
    void Parser::parse_generic_start_tag(const TagToken &tag) {
        ...
        if (t == "body") {
            if (stack_.size() < 2 || stack_[1]->tag_name != "body")
                return;
            frameset_ok_ = false;
            for (const auto &attr : tag.attributes) {
        ...
        if (t == "address" || t == "article" || t == "aside" || t == "blockquote" || t == "center" || t == "details" ||
            t == "dialog" || t == "dir" || t == "div" || t == "dl" || t == "fieldset" || t == "figcaption" ||
            t == "figure" || t == "footer" || t == "header" || t == "hgroup" || t == "main" || t == "nav" ||
            t == "ol" || t == "p" || t == "section" || t == "summary" || t == "ul") {
        ...
        if (t == "b" || t == "big" || t == "code" || t == "em" || t == "font" || t == "i" || t == "s" || t == "small" ||
            t == "strike" || t == "strong" || t == "tt" || t == "u") {
            auto *el = create_element_for_token(tag);
            insert_element(el);
            stack_.push_back(el);
            push_active_formatting_element(el);
    ...
    void Parser::parse_generic_end_tag(const TagToken &tag) {
        ...
        if (t == "a" || t == "b" || t == "big" || t == "code" || t == "em" || t == "font" || t == "i" || t == "nobr" ||
        ...
        if (t == "svg" || t == "math") {
            for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
                if (stack_[static_cast<u32>(i)] && stack_[static_cast<u32>(i)]->tag_name == t) {
                    stack_.resize(static_cast<u32>(i));
                    break;
                }
            }
            ...
            foreign_ = false;
            for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
                if (stack_[static_cast<u32>(i)] && (stack_[static_cast<u32>(i)]->tag_name == "svg" ||
        ...
        if (t == "div" || t == "section" || t == "nav" || t == "article" || t == "aside" || t == "header" ||
            t == "footer" || t == "main" || t == "address" || t == "blockquote" || t == "center" || t == "details" ||
            t == "dialog" || t == "dir" || t == "dl" || t == "fieldset" || t == "figcaption" || t == "figure" ||
            t == "hgroup" || t == "listing" || t == "menu" || t == "pre" || t == "summary" || t == "ul" || t == "ol") {
            if (!has_element_in_scope(t))
                return;
            generate_implied_end_tags({t});
            close_element(t);
        ...
        for (i32 i = static_cast<i32>(stack_.size()) - 1; i >= 0; i--) {
            if (stack_[i] && stack_[i]->tag_name == t) {
                generate_implied_end_tags({t});
                stack_.resize(static_cast<u32>(i));
                // Re-evaluate foreign context after popping
                foreign_ = false;
                for (i32 j = static_cast<i32>(stack_.size()) - 1; j >= 0; j--) {
                    if (stack_[j] && (stack_[j]->tag_name == "svg" || stack_[j]->tag_name == "math")) {
                        foreign_ = true;
                        break;
                    }
                }
