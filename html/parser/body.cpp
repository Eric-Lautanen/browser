...
namespace browser::html {
    ...
    void Parser::handle_in_body(const Token &token) {
        ...
        // Spec ("in body"): start tags caption/col/colgroup/frame/head/tbody/td/
        // tfoot/th/thead/tr are parse errors — ignore them. Same list of end tags
        // (plus body/html handled elsewhere) are ignored as well.
        static const char *k_table_ignored[] = {
            "caption", "col", "colgroup", "frame", "head", "tbody", "td", "tfoot", "th", "thead", "tr"};
        {
            const std::string &t = tag.tag_name;
            for (const char *ignored : k_table_ignored) {
                if (t == ignored)
                    return;
            }
        }
        ...
        if (token.index() == 1) {
            ...
            if (tag.type == TokenType::START_TAG) {
                ...
                if (tag.tag_name == "table") {
                    // Spec: nested <table> — pop elements until the current table
                    // has been popped, reset the insertion mode and reprocess so
                    // the inner table becomes a sibling of the outer one.
    ...
    void Parser::handle_in_table_body(const Token &token) {
        ...
        if (token.index() == 1) {
            ...
            if (tag.type == TokenType::END_TAG) {
                if (tag.tag_name == "tbody" || tag.tag_name == "tfoot" || tag.tag_name == "thead") {
                    if (!has_element_in_table_scope(tag.tag_name))
                        return;
                    clear_stack_back_to_table_body_context();
                    stack_.pop_back();
                    mode_ = InsertionMode::IN_TABLE;
        ...
        // Spec ("in table body"): anything else — process using "in table" rules,
        // which enable foster parenting for out-of-table content.
        foster_parenting_ = true;
    ...
    void Parser::handle_in_row(const Token &token) {
        ...
        // Spec ("in row"): anything else — process using "in table" rules.
        foster_parenting_ = true;
        handle_in_table(token);
        foster_parenting_ = false;
    }
        ...
        if (token.index() == 1) {
            ...
            if (tag.type == TokenType::END_TAG) {
                if (tag.tag_name == "td" || tag.tag_name == "th") {
                    ...
                    stack_.pop_back();
                    if (!active_formatting_elements_.empty() && active_formatting_elements_.back() == nullptr) {
    ...
    void Parser::handle_in_column_group(const Token &token) {
        ...
        if (token.index() == 1) {
            ...
            if (tag.type == TokenType::END_TAG) {
                if (tag.tag_name == "colgroup") {
                    if (!current_node() || current_node()->tag_name != "colgroup")
                        return;
                    stack_.pop_back();
                    mode_ = InsertionMode::IN_TABLE;
                    return;
