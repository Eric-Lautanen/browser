...
namespace browser::html {
    ...
    void Parser::adoption_agency_algorithm(const std::string &subject) {
        ...
        static const std::unordered_set<std::string> special_tags = {
            "address",    "blockquote",   "center",       "dir",     "div",  "dl",    "fieldset",  "figure",
            "figcaption", "footer",       "header",       "hgroup",  "main", "nav",   "ol",        "p",
            "section",    "ul",           "pre",          "listing", "form", "table", "table-row", "table-cell",
            "table-body", "table-header", "table-footer", "hr",      "li",   "dd",    "dt",        "h1",
            "h2",         "h3",           "h4",           "h5",      "h6"};
        ...
        for (int outer = 0; outer < 8; outer++) {
            ...
            if (formatting_stack_index < 0) {
                active_formatting_elements_.erase(active_formatting_elements_.begin() + formatting_index);
                return;
            }
            ...
            // Move all children of furthest_block into the clone, then append clone to furthest_block
            Node *fb_node = stack_[static_cast<u32>(furthest_block)];
            if (fb_node) {
                for (auto &child : fb_node->children) {
                    child->parent = formatting_clone;
                    formatting_clone->children.push_back(std::move(child));
                }
                fb_node->children.clear();
                append_child(fb_node, std::unique_ptr<Node>(static_cast<Node *>(formatting_clone)));
            }

            // Add clone to stack after furthest block and to active list
            stack_.insert(stack_.begin() + furthest_block + 1, formatting_clone);
    ...
    void Parser::push_active_formatting_element(Element *el) {
        ...
        for (i32 i = static_cast<i32>(active_formatting_elements_.size()) - 1; i >= 0; i--) {
            ...
            if (active_formatting_elements_[static_cast<u32>(i)] &&
                ...
                active_formatting_elements_[static_cast<u32>(i)]->attributes == el->attributes) {
                count++;
            }
            if (count >= 3) {
                active_formatting_elements_.erase(active_formatting_elements_.begin() + static_cast<i64>(i));
                break;
            }
        }
