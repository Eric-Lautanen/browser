...
namespace browser::net {
    ...
    bool CookieJar::path_match(const std::string &cookie_path, const std::string &request_path) {
        ...
        if (request_path.substr(0, cookie_path.size()) != cookie_path)
            return false;
        if (request_path.size() > cookie_path.size() && cookie_path.back() != '/' &&
    ...
    void CookieJar::set_cookie(const std::string &request_domain, const std::string &request_path, const Cookie &c) {
        ...
        for (auto it = cookies_.begin(); it != cookies_.end();) {
            if (it->name == cookie.name && to_lower(it->domain) == to_lower(cookie.domain) && it->path == cookie.path) {
    ...
    std::vector<Cookie> CookieJar::get_cookies(const std::string &domain,
        ...
        for (auto &c : cookies_) {
            if (is_expired(c))
                continue;
            if (!domain_match(c.domain, domain))
                continue;
            if (!path_match(c.path, path))
                continue;
            if (c.secure && !secure_only)
                continue;
    ...
    std::vector<Cookie> CookieJar::get_cookies_for_js(const std::string &domain,
                                                      const std::string &path,
                                                      bool secure_only) const {
        std::lock_guard<std::mutex> lock(mutex_);
        std::vector<Cookie> result;
    ...
    Result<void> CookieJar::load_from_file(const std::string &path) {
        ...
        while (std::getline(f, line)) {
            ...
            if (!std::getline(ss, domain, '\t'))
                continue;
            if (!std::getline(ss, domain_flag, '\t'))
                continue;
            if (!std::getline(ss, path_str, '\t'))
                continue;
            if (!std::getline(ss, secure_flag, '\t'))
                continue;
            if (!std::getline(ss, expires_str, '\t'))
                continue;
    ...
    Result<void> CookieJar::save_to_file(const std::string &path) {
        ...
        for (auto &c : cookies_) {
            ...
            f << c.domain << "\t" << domain_flag << "\t" << c.path << "\t" << secure_flag << "\t" << expires_str << "\t"
              << c.name << "\t" << c.value << "\n";
        }
