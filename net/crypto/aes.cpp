#include "aes.hpp"

namespace browser::net::crypto {

    static const u8 SBOX[256] = {
        };

    static const u32 RCON[11] = {0,
                                 0x01000000,
                                 0x02000000,
                                 0x04000000,
                                 0x08000000,
                                 0x10000000,
                                 0x20000000,
                                 0x40000000,
                                 0x80000000,
                                 0x1B000000,
                                 0x36000000};

    AES::AES() : nk_(0), nr_(0) {}

    void AES::set_key(const u8 *key, std::size_t key_len) {
        if (key_len == 16) {
            nk_ = 4;
            nr_ = 10;
        } else if (key_len == 24) {
            nk_ = 6;
            nr_ = 12;
        } else if (key_len == 32) {
            nk_ = 8;
            nr_ = 14;
        } else {
            return;
        }
        key_len_ = key_len;
        std::memcpy(key_, key, key_len_);
        key_expansion();
    }

    void AES::set_iv(const u8 *iv, std::size_t iv_len) {
        iv_len_ = iv_len < 12 ? iv_len : 12;
        std::memcpy(iv_, iv, iv_len_);
    }

    u32 AES::load_word(const u8 *p) {
        return (static_cast<u32>(p[0]) << 24) | (static_cast<u32>(p[1]) << 16) | (static_cast<u32>(p[2]) << 8) |
               (static_cast<u32>(p[3]));
    }

    void AES::store_word(u8 *p, u32 w) {
        p[0] = static_cast<u8>(w >> 24);
        p[1] = static_cast<u8>(w >> 16);
        p[2] = static_cast<u8>(w >> 8);
        p[3] = static_cast<u8>(w);
    }

    u32 AES::sub_word(u32 w) {
        return (static_cast<u32>(SBOX[(w >> 24) & 0xFF]) << 24) | (static_cast<u32>(SBOX[(w >> 16) & 0xFF]) << 16) |
               (static_cast<u32>(SBOX[(w >> 8) & 0xFF]) << 8) | (static_cast<u32>(SBOX[w & 0xFF]));
    }

    u32 AES::rot_word(u32 w) {
        return (w << 8) | (w >> 24);
    }

    u32 AES::xtime(u32 x) {
        return ((x << 1) ^ ((x >> 7) * 0x1B)) & 0xFF;
    }

    void AES::xor_block(const u8 a[16], const u8 b[16], u8 out[16]) {
        for (int i = 0; i < 16; i++) {
            out[i] = a[i] ^ b[i];
        }
    }

    void AES::key_expansion() {
        for (std::size_t i = 0; i < nk_; i++) {
            rk_[i] = load_word(key_ + i * 4);
        }

        for (std::size_t i = nk_; i < 4 * (nr_ + 1); i++) {
            u32 temp = rk_[i - 1];
            if (i % nk_ == 0) {
                temp = sub_word(rot_word(temp)) ^ RCON[i / nk_];
            } else if (nk_ == 8 && i % nk_ == 4) {
                temp = sub_word(temp);
            }
            rk_[i] = rk_[i - nk_] ^ temp;
        }
    }

    void AES::encrypt_block(const u8 in[16], u8 out[16]) const {
        u8 s[4][4];
        for (int c = 0; c < 4; c++) {
            s[0][c] = in[c * 4 + 0];
            s[1][c] = in[c * 4 + 1];
            s[2][c] = in[c * 4 + 2];
            s[3][c] = in[c * 4 + 3];
        }

        auto add_rk = [&](std::size_t r) {
            for (int c = 0; c < 4; c++) {
                u32 w = rk_[r * 4 + c];
                s[0][c] ^= static_cast<u8>(w >> 24);
                s[1][c] ^= static_cast<u8>(w >> 16);
                s[2][c] ^= static_cast<u8>(w >> 8);
                s[3][c] ^= static_cast<u8>(w);
            }
        };

        add_rk(0);
        for (std::size_t r = 1; r < nr_; r++) {
            for (int c = 0; c < 4; c++)
                for (int i = 0; i < 4; i++) s[i][c] = SBOX[s[i][c]];

            u8 t = s[1][0];
            s[1][0] = s[1][1];
            s[1][1] = s[1][2];
            s[1][2] = s[1][3];
            s[1][3] = t;
            t = s[2][0];
            s[2][0] = s[2][2];
            s[2][2] = t;
            t = s[2][1];
            s[2][1] = s[2][3];
            s[2][3] = t;
            t = s[3][3];
            s[3][3] = s[3][2];
            s[3][2] = s[3][1];
            s[3][1] = s[3][0];
            s[3][0] = t;

            for (int c = 0; c < 4; c++) {
                u8 a0 = s[0][c], a1 = s[1][c], a2 = s[2][c], a3 = s[3][c];
                s[0][c] = xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3;
                s[1][c] = a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3;
                s[2][c] = a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3);
                s[3][c] = (xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3);
            }

            add_rk(r);
        }

        for (int c = 0; c < 4; c++)
            for (int i = 0; i < 4; i++) s[i][c] = SBOX[s[i][c]];

        u8 t = s[1][0];
        s[1][0] = s[1][1];
        s[1][1] = s[1][2];
        s[1][2] = s[1][3];
        s[1][3] = t;
        t = s[2][0];
        s[2][0] = s[2][2];
        s[2][2] = t;
        t = s[2][1];
        s[2][1] = s[2][3];
        s[2][3] = t;
        t = s[3][3];
        s[3][3] = s[3][2];
        s[3][2] = s[3][1];
        s[3][1] = s[3][0];
        s[3][0] = t;

        add_rk(nr_);

        for (int c = 0; c < 4; c++) {
            out[c * 4 + 0] = s[0][c];
            out[c * 4 + 1] = s[1][c];
            out[c * 4 + 2] = s[2][c];
            out[c * 4 + 3] = s[3][c];
        }
    }

    void AES::gcm_incr(u8 *x) const {
        for (int i = 15; i >= 12; i--) {
            if (++x[i] != 0)
                break;
        }
    }

    void AES::gf128_mul(const u8 x[16], const u8 y[16], u8 z[16]) const {
        u8 v[16];
        std::memcpy(v, y, 16);
        std::memset(z, 0, 16);

        // N-P3/N-S7: the reduction is branchless — each of the 128 secret bits
        // becomes a full-width mask instead of a data-dependent branch.
        for (int i = 0; i < 128; i++) {
            int byte_idx = i >> 3;
            int bit_idx = 7 - (i & 7);
            u8 mask = static_cast<u8>(-static_cast<int>((x[byte_idx] >> bit_idx) & 1));
            for (int j = 0; j < 16; j++) z[j] ^= v[j] & mask;

            int carry = v[15] & 1;
            for (int j = 15; j > 0; j--) {
                v[j] = (v[j] >> 1) | (v[j - 1] << 7);
            }
            v[0] >>= 1;

            if (carry) {
                v[0] ^= 0xE1;
            }
        }
    }

    void AES::derive_j0(u8 j0[16]) const {
        std::memset(j0, 0, 16);
        if (iv_len_ == 12) {
            std::memcpy(j0, iv_, 12);
            j0[15] = 1;
            return;
        }
        // For non-96-bit IVs, J0 = GHASH(H, IV || 0^s || len(IV)*8)
        u8 H[16] = {0};
        encrypt_block(H, H);

        u8 y[16] = {0};
        auto feed = [&](const u8 *p, std::size_t n) {
            while (n >= 16) {
                u8 tmp[16];
                xor_block(y, p, tmp);
                gf128_mul(tmp, H, y);
                p += 16;
                n -= 16;
            }
            if (n > 0) {
                u8 block[16] = {0};
                std::memcpy(block, p, n);
                u8 tmp[16];
                xor_block(y, block, tmp);
                gf128_mul(tmp, H, y);
            }
        };
        feed(iv_, iv_len_);

        u8 lenblock[16] = {0};
        u64 iv_bits = static_cast<u64>(iv_len_) * 8;
        for (int i = 7; i >= 0; i--) lenblock[7 - i] = static_cast<u8>(iv_bits >> (i * 8));
        u8 tmp[16];
        xor_block(y, lenblock, tmp);
        gf128_mul(tmp, H, y);

        std::memcpy(j0, y, 16);
    }

    void AES::gcm_tag_from_ghash(const u8 j0[16], const u8 s[16], u8 tag[16]) const {
        u8 encrypted_j0[16];
        encrypt_block(j0, encrypted_j0);
        for (int i = 0; i < 16; i++) tag[i] = s[i] ^ encrypted_j0[i];
    }

    void AES::gcm_ghash(
        const u8 *aad, std::size_t aad_len, const u8 *ciphertext, std::size_t ct_len, u8 out[16]) const {
        // N-P3: stream over the caller's buffers. The previous version
        // concatenated AAD + padding + ciphertext + padding + lengths into one
        // vector per call (two heap copies of every record).
        u8 H[16] = {0};
        encrypt_block(H, H);

        u8 y[16] = {0};
        auto feed = [&](const u8 *p, std::size_t n) {
            while (n >= 16) {
                u8 tmp[16];
                xor_block(y, p, tmp);
                gf128_mul(tmp, H, y);
                p += 16;
                n -= 16;
            }
            if (n > 0) {
                // Zero-padded partial block: XOR with zeros is implicit.
                u8 block[16] = {0};
                std::memcpy(block, p, n);
                u8 tmp[16];
                xor_block(y, block, tmp);
                gf128_mul(tmp, H, y);
            }
        };

        feed(aad, aad_len);
        feed(ciphertext, ct_len);

        u8 lenblock[16] = {0};
        u64 aad_bits = static_cast<u64>(aad_len) * 8;
        u64 ct_bits = static_cast<u64>(ct_len) * 8;
        for (int i = 7; i >= 0; i--) lenblock[7 - i] = static_cast<u8>(aad_bits >> (i * 8));
        for (int i = 7; i >= 0; i--) lenblock[15 - i] = static_cast<u8>(ct_bits >> (i * 8));
        u8 tmp[16];
        xor_block(y, lenblock, tmp);
        gf128_mul(tmp, H, y);

        std::memcpy(out, y, 16);
    }

    void AES::gcm_gctr(const u8 *icb, const u8 *in, std::size_t in_len, u8 *out) const {
        u8 cb[16];
        std::memcpy(cb, icb, 16);

        std::size_t offset = 0;
        while (offset < in_len) {
            u8 encrypted[16];
            encrypt_block(cb, encrypted);

            std::size_t remaining = in_len - offset;
            std::size_t block_size = remaining < 16 ? remaining : 16;
            for (std::size_t j = 0; j < block_size; j++) {
                out[offset + j] = in[offset + j] ^ encrypted[j];
            }
            offset += block_size;

            gcm_incr(cb);
        }
    }

    std::vector<u8> AES::encrypt_gcm(const u8 *data, std::size_t len, const u8 *aad, std::size_t aad_len, u8 tag[16]) {
        u8 J0[16];
        derive_j0(J0);

        u8 incr_j0[16];
        std::memcpy(incr_j0, J0, 16);
        gcm_incr(incr_j0);

        std::vector<u8> ct(len);
        gcm_gctr(incr_j0, data, len, ct.data());

        u8 S[16];
        gcm_ghash(aad, aad_len, ct.data(), len, S);
        gcm_tag_from_ghash(J0, S, tag);

        return ct;
    }

    bool AES::decrypt_gcm(const u8 *data, std::size_t len, const u8 *aad, std::size_t aad_len, const u8 tag[16]) {
        u8 J0[16];
        derive_j0(J0);

        u8 S[16];
        gcm_ghash(aad, aad_len, data, len, S);

        u8 expected_tag[16];
        gcm_tag_from_ghash(J0, S, expected_tag);

        u8 diff = 0;
        for (int i = 0; i < 16; i++) {
            diff |= expected_tag[i] ^ tag[i];
        }

        return diff == 0;
    }

    std::vector<u8> AES::decrypt_gcm_return(
        const u8 *data, std::size_t len, const u8 *aad, std::size_t aad_len, const u8 tag[16]) {
        u8 J0[16];
        derive_j0(J0);

        u8 S[16];
        gcm_ghash(aad, aad_len, data, len, S);

        u8 expected_tag[16];
        gcm_tag_from_ghash(J0, S, expected_tag);

        u8 diff = 0;
        for (int i = 0; i < 16; i++) {
            diff |= expected_tag[i] ^ tag[i];
        }
        if (diff != 0)
            return {};

        // GCTR decrypt (CTR mode is its own inverse)
        u8 incr_j0[16];
        std::memcpy(incr_j0, J0, 16);
        gcm_incr(incr_j0);

        std::vector<u8> pt(len);
        gcm_gctr(incr_j0, data, len, pt.data());
        return pt;
    }

}  // namespace browser::net::crypto
