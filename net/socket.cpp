...
Result<IPv4Address> IPv4Address::from_string(const std::string& s) {
    ...
    for (char c : s) {
        if (c == '.') {
            if (count >= 4) return std::string("invalid IPv4 address");
    ...
    if (count != 3 || current.empty()) return std::string("invalid IPv4 address");
    parts[count] = static_cast<u32>(std::stoul(current));
    for (int i = 0; i < 4; i++) {
        if (parts[i] > 255) return std::string("invalid IPv4 address");
...
Result<IPv6Address> IPv6Address::from_string(const std::string& s) {
    ...
    auto flush = [&]() -> Result<void> {
        ...
        if (num_parsed >= 8) return std::string("too many IPv6 groups");
