...
private:
    ...
    // Handshake transcript and incremental hasher
    std::vector<u8> transcript_;
    crypto::SHA256 transcript_hasher_;

    // Traffic secrets (32-byte keys for ChaCha20, 16-byte for AES — use 32 for both)
    ...
    // Cached AES key schedules (avoid re-expansion per AEAD call)
