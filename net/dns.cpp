...
static Result<std::pair<std::string, u16>> read_dns_name(const u8* data, u32 len, u32 offset) {
    ...
    while (pos < len) {
        if (labels++ > max_labels) {
            return std::string("DNS name too many labels");
        }
        u8 b = data[pos];
        if (b == 0) {
            if (!jumped) {
                consumed = static_cast<u16>(pos - offset + 1);
            }
            pos++;
            break;
        }
        if ((b & 0xC0) == 0xC0) {
            if (pos + 1 >= len) {
                return std::string("truncated DNS pointer");
            }
            u16 ptr = ((static_cast<u16>(b & 0x3F)) << 8) | static_cast<u16>(data[pos + 1]);
            if (ptr >= len) {
                return std::string("DNS pointer out of bounds");
            }
            if (!jumped) {
                consumed = static_cast<u16>(pos - offset + 2);
                jumped = true;
            }
            pos = ptr;
            continue;
        ...
        if (pos + 1 + label_len > len) {
            return std::string("truncated DNS label");
        }
        if (!name.empty()) name += '.';
        for (u8 i = 0; i < label_len; i++) {
            name += static_cast<char>(data[pos + 1 + i]);
        }
        pos += 1 + label_len;
    }
...
Result<std::vector<IPv4Address>> DNSResolver::resolve_a(const std::string& hostname) {
    u16 id = next_tid_++;
    auto query = build_query(hostname, 1, id);

    auto sr = ensure_socket();
    if (sr.is_err()) return sr.unwrap_err();

    auto sends = sock_->send(query.data(), static_cast<u32>(query.size()), dns_server_, 53);
    if (sends.is_err()) {
        return std::string("DNS send failed");
    }

    u8 recv_buf[1500];
    IPv4Address sender;
    u16 sender_port;
    auto rr = sock_->receive(recv_buf, sizeof(recv_buf), &sender, &sender_port);
    if (rr.is_err()) {
        return std::string("DNS receive failed");
    }
    u32 recv_len = rr.unwrap();

    return parse_response(recv_buf, recv_len, id);
}
...
Result<std::vector<IPv4Address>> DNSResolver::parse_response(const u8* data, u32 len, u16 expected_id) {
    ...
    if (len < 12) {
        return std::string("DNS response too short");
    }

    u16 id = (static_cast<u16>(data[0]) << 8) | data[1];
    if (id != expected_id) {
        return std::string("DNS response ID mismatch");
    }

    u16 flags = (static_cast<u16>(data[2]) << 8) | data[3];
    if ((flags & 0x8000) == 0) {
        return std::string("DNS response is not a response");
    }
    u8 rcode = flags & 0x0F;
    if (rcode != 0) {
        return std::string("DNS response error code: ") + std::to_string(rcode);
    }
    ...
    for (u16 i = 0; i < qdcount; i++) {
        ...
        if (pos + 4 > len) {
            return std::string("truncated DNS question");
        }
        pos += 4;
    }
    ...
    for (u16 i = 0; i < ancount; i++) {
        if (pos >= len) {
            return std::string("truncated DNS answer section");
        }
        ...
        if (pos + 10 > len) {
            return std::string("truncated DNS RR header");
        }

        u16 rtype = (static_cast<u16>(data[pos]) << 8) | data[pos + 1];
        u16 rclass = (static_cast<u16>(data[pos + 2]) << 8) | data[pos + 3];
        (void)rclass;
        u32 rttl = (static_cast<u32>(data[pos + 4]) << 24) |
                   (static_cast<u32>(data[pos + 5]) << 16) |
                   (static_cast<u32>(data[pos + 6]) << 8) |
                   static_cast<u32>(data[pos + 7]);
        (void)rttl;
        u16 rdlength = (static_cast<u16>(data[pos + 8]) << 8) | data[pos + 9];
        pos += 10;

        if (pos + rdlength > len) {
            return std::string("truncated DNS RDATA");
        }
