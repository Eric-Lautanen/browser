...
namespace browser::net::http {
    ...
    std::vector<u8> Request::serialize() const {
        static const char *method_names[] = {"GET", "POST", "HEAD", "PUT", "DELETE", "CONNECT", "OPTIONS", "PATCH"};
        int mi = static_cast<int>(method);
        const char *mn = (mi >= 0 && mi < 8) ? method_names[mi] : "GET";

        std::string req_line = std::string(mn) + " " + (url.path.empty() ? "/" : url.path) +
    ...
    Result<Response> Response::parse(const u8 *data, u32 len) {
        ...
        if (status_line.empty())
            return std::string("empty status line");

        // Parse "HTTP/x.x" version
        auto sp1 = status_line.find(' ');
        if (sp1 == std::string::npos)
            return std::string("bad status line");
        resp.http_version = status_line.substr(0, sp1);

        // Parse status code
        auto sp2 = status_line.find(' ', sp1 + 1);
        if (sp2 == std::string::npos)
            return std::string("bad status line: missing reason");
        ...
        if (chunked) {
            ...
            while (pos < len) {
                ...
                for (char c : hex_size) {
                    chunk_size *= 16;
                    if (c >= '0' && c <= '9')
                        chunk_size += static_cast<u32>(c - '0');
                    else if (c >= 'a' && c <= 'f')
                        chunk_size += static_cast<u32>(c - 'a' + 10);
                    else
                        chunk_size += static_cast<u32>(c - 'A' + 10);
                }
    ...
    Result<void> HTTP1Client::connect(const std::string &host,
                                      u16 port,
                                      bool use_tls,
    ...
    Result<void> HTTP1Client::connect_if_needed(const Request &req) {
        ...
        if (tcp_.valid() && tcp_->is_open() && tcp_->host() == host && tcp_->port() == port) {
    ...
    Result<Response> HTTP1Client::execute(const Request &req) {
        ...
        if (cr.is_err())
            return std::string("execute: " + cr.unwrap_err());
        ...
        if (use_tls_) {
            ...
            if (sr.is_err())
                return std::string("send: " + sr.unwrap_err());
        } else {
    ...
    async::task<Response> HTTP1Client::execute_async(const Request &req) {
        auto cr = connect_if_needed(req);
        if (cr.is_err())
            co_return std::string("execute: ") + cr.unwrap_err();
