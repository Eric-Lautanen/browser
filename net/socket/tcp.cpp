...
namespace browser::net {
    ...
    Result<void> Win32TCPSocket::load_connect_ex() {
        if (connect_ex_func_)
            return {};
        if (handle_ == INVALID_SOCKET)
            return std::string("socket not created");
        GUID guid = WSAID_CONNECTEX;
        DWORD bytes = 0;
        int ret = WSAIoctl(handle_,
                           SIO_GET_EXTENSION_FUNCTION_POINTER,
                           &guid,
                           sizeof(guid),
                           &connect_ex_func_,
                           sizeof(connect_ex_func_),
                           &bytes,
                           nullptr,
                           nullptr);
        if (ret == SOCKET_ERROR) {
    ...
    Result<void> Win32TCPSocket::connect(const std::string &host, u16 port) {
        ...
        handle_ =
            ::WSASocket(result->ai_family, result->ai_socktype, result->ai_protocol, nullptr, 0, WSA_FLAG_OVERLAPPED);
        if (handle_ == INVALID_SOCKET) {
            ::freeaddrinfo(result);
    ...
    Result<void> Win32TCPSocket::connect_ip(const IPv4Address &addr, u16 port) {
        ...
        u32 ip = (static_cast<u32>(addr.octets[0]) << 24) | (static_cast<u32>(addr.octets[1]) << 16) |
                 (static_cast<u32>(addr.octets[2]) << 8) | static_cast<u32>(addr.octets[3]);
        server_addr.sin_addr.s_addr = htonl(ip);

        int ret = ::connect(
            handle_, reinterpret_cast<struct sockaddr *>(&server_addr), static_cast<int>(sizeof(server_addr)));
        if (ret == SOCKET_ERROR) {
            int err = WSAGetLastError();
            if (err != WSAEWOULDBLOCK) {
                ::closesocket(handle_);
    ...
    Result<u32> Win32TCPSocket::send(const u8 *data, u32 len) {
        ...
        if (ret == SOCKET_ERROR)
            return std::string("WSASend failed");
    ...
    Result<u32> Win32TCPSocket::receive(u8 *buf, u32 len) {
        ...
        if (ret == SOCKET_ERROR)
            return std::string("WSARecv failed");
    ...
    async::task<bool> Win32TCPSocket::async_connect(const std::string &host, u16 port) {
        ...
        if (handle_ == INVALID_SOCKET) {
            ::freeaddrinfo(result);
            co_return std::string("WSASocket failed");
        }
        ...
        if (assoc_r.is_err()) {
            ...
            ::closesocket(handle_);
            handle_ = INVALID_SOCKET;
            co_return std::string("IOCP: ") + assoc_r.unwrap_err();
        }

        GUID guid = WSAID_CONNECTEX;
        DWORD bytes = 0;
        LPFN_CONNECTEX connect_ex = nullptr;
        int wret = WSAIoctl(handle_,
                            SIO_GET_EXTENSION_FUNCTION_POINTER,
                            &guid,
                            sizeof(guid),
                            &connect_ex,
        ...
        if (ol.error) {
            ::closesocket(handle_);
            handle_ = INVALID_SOCKET;
            co_return std::string("ConnectEx error");
        }
        co_return true;
    }
