...
namespace browser::test {
    ...
    namespace detail {
        inline std::string to_string_impl(bool v) {
            return v ? "true" : "false";
        }
        template <typename T>
...
#define TEST(name, ...)                                                                                \
    static bool _test_##name = [] {                                                                    \
        ::browser::test::TestRegistry::instance().register_test(#name, [](std::string &_err) -> bool { \
            (void)_err;                                                                                \
            __VA_ARGS__;                                                                               \
            return true;                                                                               \
        });                                                                                            \
...
#define ASSERT_EQ(a, b)                                                                    \
    if ((a) != (b)) {                                                                      \
        _err = "ASSERT_EQ failed at line " + std::to_string(__LINE__) + ": " + #a + " (" + \
               ::browser::test::detail::to_string_impl(a) + ") != " + #b + " (" +          \
               ::browser::test::detail::to_string_impl(b) + ")";                           \
        return false;                                                                      \
    }
