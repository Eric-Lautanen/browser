...
namespace browser {
    ...
    TEST(tab_placeholder_color, {
        ...
        render::Color c{0.5f + static_cast<f32>((hash >> 16) & 0xFF) / 510.0f,
    ...
    // BR-P2: the idle render gate's wake policy. An idle browser must doze far
    // longer than one animation frame; animation keeps a 16 ms beat; caret blink
    // polls faster than its 500 ms period.
    TEST(chrome_idle_wait_policy, {
        ASSERT(BrowserWindow::idle_wait_ms(true, true) == 16);
        ASSERT(BrowserWindow::idle_wait_ms(true, false) == 16);
        ASSERT(BrowserWindow::idle_wait_ms(false, true) == 60);
        u32 idle = BrowserWindow::idle_wait_ms(false, false);
        // Idle must be meaningfully cheaper than a 60 Hz frame budget (16.6 ms)
        // or the dirty gate saves no wakeups.
        ASSERT(idle > 100);
    })
