...
namespace browser::js {
    ...
    struct UnaryExpr : Node {
        ...
        TokenType op = TokenType::EOF_TOKEN;
        bool prefix = true;
        // Keyword unary operators (typeof / void / delete): IDENTIFIER tokens
        // carry no distinguishing TokenType, so the keyword text is kept here.
        std::string op_name;
    };

    // J-C6: conditional (ternary) expression — test ? consequent : alternate.
    struct ConditionalExpr : Node {
    ...
    struct Expr : std::variant<LiteralExpr,
                               IdentExpr,
                               BinaryExpr,
                               UnaryExpr,
                               CallExpr,
    ...
    // J-C6: continue statement for loops.
    ...
    struct Stmt : std::variant<ExpressionStmt,
                               BlockStmt,
                               IfStmt,
                               WhileStmt,
                               ForStmt,
    ...
    struct Pattern : std::variant<IdentPattern, ObjPattern, ArrPattern> {
        using variant::variant;
        Pattern() = default;
    };
