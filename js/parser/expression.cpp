...
namespace browser::js {
    ...
    static bool is_assignment_op(TokenType type) {
        return type == TokenType::EQUALS || type == TokenType::PLUS_EQ || type == TokenType::MINUS_EQ ||
               type == TokenType::STAR_EQ || type == TokenType::SLASH_EQ || type == TokenType::PERCENT_EQ;
    }
    ...
    std::unique_ptr<Expr> Parser::parse_postfix_expr(std::unique_ptr<Expr> left, bool allow_call) {
        if (!left)
            return nullptr;
        ...
        for (;;) {
            if (current_.type == TokenType::DOT) {
                ...
                mem->computed = true;
                left = std::make_unique<Expr>(std::move(*mem));
            } else if (allow_call && current_.type == TokenType::LPAREN) {
                advance();
    ...
    std::unique_ptr<Expr> Parser::parse_primary() {
        ...
        switch (current_.type) {
            ...
            case TokenType::LPAREN: {
                ...
                bool could_be_arrow = (current_.type == TokenType::IDENTIFIER || current_.type == TokenType::LBRACE ||
                ...
                if (could_be_arrow) {
                    ...
                    if (current_.type == TokenType::COMMA) {
                        ...
                        while (match(TokenType::COMMA)) {
                            ...
                            if (p)
                                arrow_params.push_back(std::move(p));
                        }
                        if (current_.type == TokenType::RPAREN) {
                            advance();
                            if (current_.type == TokenType::ARROW) {
            ...
            case TokenType::IDENTIFIER: {
                ...
                if (current_.type == TokenType::ARROW) {
                    ...
                    arrow->line = line;
                    arrow->column = col;
                    arrow->params.push_back(std::make_unique<Pattern>(IdentPattern{name}));
                    parse_arrow_body(arrow.get());
                    return std::make_unique<Expr>(std::move(*arrow));
                }
    ...
    std::unique_ptr<Expr> Parser::parse_new_expr() {
        ...
        // Call arguments are parsed below — postfix must not consume the '(' here,
        // otherwise `new Foo.Bar()` becomes Call(Call(Member)) instead of a
        // single construction call.
        callee = parse_postfix_expr(std::move(callee), /*allow_call=*/false);
    ...
    std::unique_ptr<Expr> Parser::parse_template() {
        ...
        if (current_.type == TokenType::TEMPLATE_HEAD) {
            ...
            while (current_.type != TokenType::TEMPLATE_TAIL && current_.type != TokenType::EOF_TOKEN) {
                auto expr = parse_expression(0);
                if (expr) {
