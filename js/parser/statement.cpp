...
namespace browser::js {
    ...
    std::unique_ptr<Stmt> Parser::parse_block() {
        ...
        while (current_.type != TokenType::RBRACE && current_.type != TokenType::EOF_TOKEN) {
            u32 line_before = current_.line;
            u32 col_before = current_.column;
            auto stmt = parse_statement();
            if (stmt) {
                block->body.push_back(std::move(stmt));
            } else if (current_.line == line_before && current_.column == col_before) {
                // parse_statement consumed nothing: force progress.
                advance();
            }
        }
    ...
    std::unique_ptr<Stmt> Parser::parse_var_declaration() {
        ...
        if (current_.text == "var")
            stmt->kind = VarDeclStmt::Kind::VAR;
        else if (current_.text == "let")
            stmt->kind = VarDeclStmt::Kind::LET;
        else
            stmt->kind = VarDeclStmt::Kind::CONST;
        advance();
    ...
    std::unique_ptr<Stmt> Parser::parse_return() {
        ...
        if (!line_break && current_.type != TokenType::SEMICOLON && current_.type != TokenType::RBRACE &&
