...
namespace browser::platform {
    ...
    namespace {
        ...
        bool init_wgl_extensions() {
            ...
            HWND temp_hwnd = CreateWindowEx(
                0, kTempClassName, nullptr, WS_POPUP, 0, 0, 1, 1, nullptr, nullptr, GetModuleHandle(nullptr), nullptr);
            if (!temp_hwnd) {
            ...
            wglCreateContextAttribsARB =
                reinterpret_cast<wglCreateContextAttribsARBProc>(reinterpret_cast<void *>(proc1));
    ...
    void Win32Window::set_fullscreen(bool fullscreen) {
        ...
        if (fullscreen) {
            ...
            SetWindowLong(
                hwnd_, GWL_STYLE, saved_style_ & ~(WS_CAPTION | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX));
    ...
    void Win32Window::toggle_maximize() {
        if (maximized_) {
            SetWindowPos(hwnd_,
                         HWND_TOP,
                         pre_maximize_rect_.left,
                         pre_maximize_rect_.top,
                         pre_maximize_rect_.right - pre_maximize_rect_.left,
                         pre_maximize_rect_.bottom - pre_maximize_rect_.top,
                         SWP_FRAMECHANGED | SWP_NOZORDER);
            maximized_ = false;
            fullscreen_ = false;
        } else {
            GetWindowRect(hwnd_, &pre_maximize_rect_);
